...
<div class="tutor-dashboard-setting-profile tutor-dashboard-content-inner">
	...
	<form action="" method="post" enctype="multipart/form-data">
		...
		<div class="tutor-row">
			<div class="tutor-col-12 tutor-col-sm-6 tutor-col-md-12 tutor-col-lg-6 tutor-mb-32">
				<label class="tutor-form-label tutor-color-secondary">
					<?php esc_html_e( 'Institution/Place of Work', 'tutor' ); ?>
				</label>
				<input class="tutor-form-control" type="text" name="institution" value="<?php echo get_user_meta($user->ID,'institution',true); ?>" placeholder="<?php _e('Institution/Place of Work', 'tutor'); ?>">
			</div>

			<div class="tutor-col-12 tutor-col-sm-6 tutor-col-md-12 tutor-col-lg-6 tutor-mb-32">
				<label class="tutor-form-label tutor-color-secondary">
					<?php esc_html_e( 'Country of Work', 'tutor' ); ?>
				</label>
				<input class="tutor-form-control" type="text" name="work_country" value="<?php echo get_user_meta($user->ID,'work_country',true); ?>" placeholder="<?php _e('Country of Work', 'tutor'); ?>">
			</div>			
		</div>
		...
		<div class="tutor-row">
			<div class="tutor-col-12 tutor-col-sm-6 tutor-col-md-12 tutor-col-lg-6 tutor-mb-32">
			<label class="tutor-form-label tutor-color-secondary">
				<?php esc_html_e( 'User Speciality', 'tutor' ); ?>
			</label>
			<select name="speciality" id="speciality" required>
				...
				if (have_rows('user_specialities_registration', $settingsId)) :
					while (have_rows('user_specialities_registration', $settingsId)) : the_row();
						...
						$selected_value = get_user_meta($user->ID, 'speciality', true);
						
						// Set the option as selected if it matches the value from the database
						if ($speciality === $selected_value) {
							$selected = 'selected="selected"';
						}
						?>
						<option value="<?php echo esc_attr($speciality); ?>" <?php echo $selected; ?>><?php echo esc_html($speciality); ?></option>
						<?php
					endwhile;
				endif;
				?>
		...
		<div class="tutor-row">
			<div class="tutor-col-12 tutor-col-sm-6 tutor-col-md-12 tutor-col-lg-6 tutor-mb-32">
				<div class="tutor-form-group">
					<label class="tutor-form-label tutor-color-secondary">
						<?php esc_html_e('Interests', 'tutor'); ?>
					</label>
					<?php
					$selected_interests = get_user_meta($user->ID, 'interests', true); // Get the selected interests from the database
					...
					if (have_rows('interests_registration', $settingsId)) :
						while (have_rows('interests_registration', $settingsId)) : the_row();
							...
							if (is_array($selected_interests) && in_array($interests, $selected_interests)) {
								$checked = 'checked="checked"';
							}
							?>
							<div>
								<input type="checkbox" name="interests[]" value="<?php echo esc_attr($interests); ?>" <?php echo $checked; ?>>
					...
					<!-- Add hidden field if no checkboxes are checked -->
					<?php if (empty($selected_interests)) : ?>
